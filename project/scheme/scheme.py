...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        values = Pair(scheme_eval(cur.rest.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    """
    # BEGIN Problem EC 2
    # Check target is symbol and pair
    validate_form(expressions, 2)
    target = expressions.first
    validate_formals(target)  
    validate_form(target, 0) 
    operator, formals, body = target.first, target.rest, expressions.rest
    macro_procedure = MacroProcedure(formals, body, env)
    env.define(operator, macro_procedure)
    return operator
    # END Problem EC 2
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN PROBLEM EC 1
        # You need to convert the iterative function to tail recursion (eval_all, do_and_from ...)
