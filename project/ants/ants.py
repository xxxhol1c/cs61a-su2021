...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cunt = 0
        while cunt < self.min_range and not ant_place.is_hive:
            cunt += 1
            ant_place = ant_place.entrance
        while self.min_range <= cunt <= self.max_range and not ant_place.is_hive:
            if ant_place.bees:
                return choose_bee(ant_place.bees)
            ant_place = ant_place.entrance
            cunt += 1
        return None   
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if  self.chew_countdown == 0:
            bee_aten = choose_bee(self.place.bees)
            if bee_aten:
                self.eat_bee(bee_aten)
        else:
    ...
    def action(self, gamestate):
        ...
        if self.is_impostor:
            ...
            while behind_place is not None:
                ant_behind = behind_place.ant
                if ant_behind:
                    if ant_behind.is_container and ant_behind.ant_contained and not ant_behind.ant_contained.is_buffed:
                        Ant.buff(ant_behind.ant_contained)
                    if not ant_behind.is_buffed: 
                        Ant.buff(ant_behind)
...
class Bee(Insect):
    ...
    def apply_status(self, status,pervious_action,length):
        ...
        def action(gamestate):
            ...
            if length == 0:
                ...
                self.scared = True
                self.direction = 1
                return pervious_action(gamestate)
            else:
               

                
                length -= 1
                return status(gamestate,pervious_action)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        d = {}
        ant_place = self.place
        while not ant_place.entrance.is_hive:
            if ant_place.ant and ant_place.ant is not self:
                d[ant_place.ant] = distance
