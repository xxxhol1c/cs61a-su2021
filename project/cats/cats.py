...
def about(topic):
    ...
    def select(paragraphs):
        ph = split(remove_punctuation(lower(paragraphs)))
        for ele in topic:
              for s in ph:
                  if ele == s:
                    return True
...
def accuracy(typed, reference):
    ...
    for i in range(min(len(reference_words),len(typed_words))):
        if(typed_words[i] == reference_words[i]):
            cnt += 1
    return float(cnt/len(typed_words))*100 
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    res  = [(s, diff_function(typed_word,s,limit)) for s in valid_words ]
    m = min(res,key=lambda x:x[1]) 

    return typed_word if m[1] > limit else m[0] 
...
def report_progress(sofar, prompt, user_id, send):
    ...
    for i in range(len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            break
    progress = float(cnt/len(prompt)) 
    json = {"id":user_id,"progress":progress}
    send(json)
    return progress
...
def time_per_word(times_per_player, words):
    ...
    for player in times_per_player:
        lst.append([player[i]-player[i-1] for i in range(1,len(player))])
...
def fastest_words(game):
    ...
    #    word_times = [time(game,p,i) for p in player_indices]
    #    idx = min(player_indices,key=lambda x:word_times[x])
    #    fastest[idx].append(word_at(game,i))
    #return fastest 
    ...
    for word_num in word_indices:
        ...
        for player_num in player_indices:
            if time(game, player_num, word_num) < min:
                min = time(game, player_num, word_num)
                min_player_num = player_num
        fastest[min_player_num].append(word_at(game,word_num))
    return fastest    
    # END PROBLEM 10        
