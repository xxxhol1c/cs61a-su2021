...
class VendingMachine:
    ...
    def add_funds (self,amount):
        ...
        if self.stock ==0:
            return 'Machine is empty. Please restock. Here is your ${0}.'.format(amount)
        else:
            self.balance += amount
...
def is_bst(t):
    ...
    def bst_max(t):
        if t.is_leaf():
            return t.label
        return max(t.label,bst_max(t.branches[len(t.branches)-1]))
...
def generate_preorder(t):
    ...
    yield t.label
    for b in t.branches:
        for items in  generate_preorder(b):
            yield items 
